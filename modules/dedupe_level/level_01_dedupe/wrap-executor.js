...
function wrapToolExecutor(executeToolCall) {
  const cache = createCallCache();
    ...
    return async function executeWithDedupe(toolName, action, args = {}) {
      ...
      if (hit !== undefined) {
        console.log(
          `[level_01_dedupe] HIT (trùng trong turn) ${toolName}:${action}`,
        );
        return {
          ...hit,
      ...
      console.log(`[level_01_dedupe] MISS ${toolName}:${action}`);
