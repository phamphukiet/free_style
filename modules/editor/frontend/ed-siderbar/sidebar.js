...
import { createFile, createFolder, getParentPath } from "@shared/fs-actions.js";
...
class ModuleSidebarElement extends LitElement {
  ...
  static properties = {
    folderPath: { type: String }, folderName: { type: String },
    items: { type: Array }, menuOpen: { state: true },
    menuX: { state: true }, menuY: { state: true },
    menuItems: { state: true }, creatingType: { state: true },
    selectedPath: { state: true }, selectedIsDirectory: { state: true },
  };

  constructor() {
    super();
    this.folderPath = null; this.folderName = ""; this.items = [];
    this.menuOpen = false; this.menuX = 0; this.menuY = 0;
    this.menuItems = []; this.creatingType = null;
    this.selectedPath = ""; this.selectedIsDirectory = false;
  }
  ...
  connectedCallback() {
    ...
    this.addEventListener("ed:select", (e) => {
      this.selectedPath = e.detail.path; this.selectedIsDirectory = e.detail.isDirectory;
    });
  ...
  handleTreeContextMenu = (e) => {
    this.menuX = e.detail.x; this.menuY = e.detail.y;
    this.menuItems = e.detail.items; this.menuOpen = true;
  };

  handleContextMenu(e) {
    e.preventDefault();
    if (!this.folderPath) return;
    this.menuX = e.clientX; this.menuY = e.clientY;
    this.menuItems = [
  ...
  handleCollapseAll() {
    this.shadowRoot.querySelectorAll("ed-tree-item").forEach((el) => {
      el.expanded = false; el.children = null;
    });
  }

  render() { return sidebarTemplate(this); }
}
